CREATE OR REPLACE FUNCTION {{SCHEMA}}.credential_utxos(_payment_credentials text [], _extended boolean DEFAULT false)
RETURNS TABLE (
  tx_hash text,
  tx_index smallint,
  address text,
  ...
  RETURN QUERY
    WITH
      _assets AS (
        ...
        LEFT JOIN {{SCHEMA}}.asset_info_cache AS aic ON aic.asset_id = ma.id
...
COMMENT ON FUNCTION {{SCHEMA}}.credential_utxos IS 'Get UTxO details for requested payment credentials'; -- noqa: LT01
